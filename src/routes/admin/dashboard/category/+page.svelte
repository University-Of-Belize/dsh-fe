	...
	<div class="main-content flex items-center justify-start h-full text-COLORBLK">
		...
		<div class="content block px-16 py-16 w-full h-full bg-transparent overflow-auto pb-40">
			<div class="flex-header flex items-center w-full flex-wrap">
				<div class="block">
					<div class="flex text-2xl font-semibold pb-2">Category Management</div>
					<div class="flex text-xl font-semibold pb-12">Product Category Management</div>
				</div>
				<div class="flex items-center justify-end flex-1">
					<div class="flex flex-col items-end space-y-2">
						<div class="btn_wrp" on:click={() => goto('/admin/dashboard/category/manage')}>
							<Button color="COLORBLK" color_t="COLORWHT1" text="New category" icon={faPlus} />
						</div>
					</div>
				</div>
			</div>
									...
									{#if category.alias != undefined && category.alias != category.name}
										...
										<ul>
											<li class="before:content-['-⠀'] flex items-center">
												<div class="font-light text-sm">{category.alias}</div>
											</li>
										</ul>
									{/if}
								</div>
