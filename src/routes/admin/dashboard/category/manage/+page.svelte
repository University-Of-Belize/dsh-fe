...
<svelte:head>
	<title>UniFood | Dashboard / Categories / Manage '{data ? data.name : 'Category'}'</title>
</svelte:head>
			...
			<div class="editGroup flex flex-col px-4 pb-8">
				...
				<form action="#" on:submit={(event) => handleSubmit(event)} class="space-y-3">
					<div class="inputgroup flex flex-wrap items-start justify-start lg:items-center">
						<div class="label flex w-full items-center justify-start space-x-2 text-lg font-light">
									...
									on:click={() => {
										...
										toast.push(
											`You have ${data.hidden ? 'unhidden' : 'hidden'} the category ${
												data.name
											}. It will be ${data.hidden ? 'visible in' : 'removed from'} search results.`
										);
									}}
								>
									<Button
