	...
	<div class="flex-header flex w-full flex-wrap items-center">
		<div class="block">
			<div class="flex pb-2 text-2xl font-semibold">
				{user_id ? (user_id == user._id ? 'My' : data ? `${data.username}'s` : "User's") : 'Create'}
				User Account
			</div>
	...
	<div class="flex w-full flex-wrap">
			...
			{#if !isNaN(user.id)}
				<div class="user_wrap w-full">
					<UserBanner
						user={data}
						tag
						tagColor={data?.staff ? 'COLORACCENTL' : 'COLORACCENTD'}
						tagColor_t={data?.staff ? 'COLORLIGHT-100' : 'COLORLIGHT-100'}
		...
		<div
			class="editPane flex hidden flex-col lg:flex-row {staff
				? 'justify-around'
				: 'justify-start'} w-full bg-COLORDARK-75 px-4 py-4"
			bind:this={editPane}
		>
